...
breaks <-  c(0,25, 50,75,100,125,150,175,200,225,250,275,300,750)#c(0, seq(25, 100, 25), seq(150, 300, 50), max(y$Tot_Biomass))
...
input_points <- data.frame(calib_meta[,c('lat','long','in.calib.sample')]) # how to add points part A
colnames(input_points) <- c('lat','lon')

pdf(paste0(Sys.Date(), 'simple_biomass_map.pdf'))
plot(
  inputData_latlon$X,
  inputData_latlon$Y,

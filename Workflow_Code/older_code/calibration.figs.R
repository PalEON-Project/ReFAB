...
for(i in 1:ncol(samples.mixed)){
  prop.quants[i,]<-quantile(samples.mixed[,i],c(.025,.5,.975),
                            na.rm=TRUE)
}
...
for(i in 1:22){
  plot(1:145,prop.quants[grep('p.true1',
       ...
       pch=21,bg='gray',main=colnames(counts)[i],
       ylim=range(prop.quants[grep('p.true1',
                                   colnames(samples.mixed))[(plot.help[i]+1):plot.help[i+1]],]),
       ylab='pollen prop',xlab='biomass')

...
pred_code <- nimbleCode({
  ...
  # }
  for(t in 1:T){
    shape.hold1[t,1:I] <- Zb[t,1:5] %*% beta1[1:5,1:I]
    shape.hold2[t,1:I] <- Zb[t,1:5] %*% beta2[1:5,1:I]
  }
  
  for(t in 1:T){
    for(i in 1:I){
      shape1[t,i] <- linexp_vec(shape.hold1[t,i])
      shape2[t,i] <- linexp_vec(shape.hold2[t,i])
    }
  }

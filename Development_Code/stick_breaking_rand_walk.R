...
pred_code <- nimbleCode({
  ...
  sigma ~ dunif(0,200) #GELMAN PAPER #5
...
counts <- Y[,rev(order(colMeans(Y)))] # Y is calibration dataset at this point
...
for(s in 1:31){ #length(unique(x.meta[,1]))
  ...
  if(length(ten.count.use)>25*20&	min(x.meta[x.meta[,1]== site_number,]$age_bacon)<1000 & 
    ...
    Y <- Y[,rev(order(colMeans(prop.use)))]
    beta1.est.real.use <- beta1.est.real[,rev(order(colMeans(prop.use)))]
    beta2.est.real.use <- beta1.est.real[,rev(order(colMeans(prop.use)))]
    ...
    constants.pred = list(beta = beta1.est.real.use, beta1 = beta2.est.real.use, I = I, J = J,
...
#samples.pred.good.inits.save[[s]] <- samples.pred.good.inits
  ...
  save(biomassCI, file="biomass.CI12.Rdata")
  save(samples.pred.save, file="samples.pred.save4.Rdata")
}

quartz()
par(mfrow=c(3,3))
for(i in 1:100){
  plot(samples.pred.10[,i],ylim=c(0,150),pch=19,cex=.5)
  points(samples.pred.100[,i],col='blue',pch=19,cex=.5)
} 

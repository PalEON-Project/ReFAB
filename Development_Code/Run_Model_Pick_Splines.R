...
betas.big = matrix(0,10,ncol(counts))
...
##### Simulated data
data.sim.cal1 = list("I" = ncol(Y), "J" = nrow(Y), "Y" = Y , "n" = rowSums(Y),"Z" = Z.big, "beta" = beta.big, "p" = p)
inits.cal1 = list(list(beta = matrix(0,nrow(beta.big),ncol(Y))),list(beta = matrix(3,nrow(beta.big),ncol(Y))))
...
beta.est.big = matrix(summary(csamp.sim.cal1.beta)$statistics[,1],30,ncol(Y))
...
##### Real data
data.real.cal1 = list("I" = ncol(counts), "J" = nrow(counts), "Y" = counts , "n" = rowSums(counts),"Z" = Z.big, "beta" = beta.big, "p" = p)
...
beta.est.big1 = matrix(summary(csamp.real.cal1.beta)$statistics[,1],30,ncol(Y))
...
#### Simulated Data
data.sim.pred1 = list("I" = ncol(Y), "DFS" = ncol(Zb.big), "J" = J, "Zb" = Zb.big, "Y" = Y , "n" = rowSums(Y), "Z" = Z.new, "beta" = beta.est.big, "p" = p, "phi.first" = phi.first)
inits.pred = list(list(b = rep(100,J)),list(b=rep(300,J)))
...
par(mfrow=c(1,1))
plot(biomass,summary(csamp.sim.pred1)$statistics[,1],main = "Real Data", xlab = "true biomass",ylab="biomass estimates",ylim=c(0,400),xlim=c(0,400))
points(biomass,summary(csamp.sim.pred1)$quantiles[,2],pch=16,col="blue")
lines(seq(0,400,1),seq(0,400,1))

#### Real Data
data.real.pred1 = list("I" = ncol(Y), "DFS" = ncol(Zb.big), "J" = J, "Zb" = Zb.big, "Y" = counts , "n" = rowSums(Y), "Z" = Z.new, "beta" = beta.est.big1, "p" = p, "phi.first" = phi.first)
